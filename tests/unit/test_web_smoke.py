...
def test_web_paste_rtf_and_download_smoke():
    ...
    assert b"software_version,conversion_ruleset" in payloads["pubmed_audit.csv"]
    assert b"0.1.0,v20" in payloads["pubmed_audit.csv"]
    assert b"line_number,original,converted" in payloads["pubmed_audit.csv"]
    assert b'"software_version": "0.1.0"' in payloads["pubmed_validation.json"]
    assert b'"conversion_ruleset": "v20"' in payloads["pubmed_validation.json"]
