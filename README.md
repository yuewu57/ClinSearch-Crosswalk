...
An **External source-search end date** appears under Advanced options only
when a combined Ovid `.ed,dt.` construct is detected. It is solely a legacy
input used to decide whether eligible Ovid update-date rows are omitted. It
does not add a PubMed publication-date restriction, and ordinary strategies
produce the same query whether or not an end date is supplied programmatically.

### RTF upload

Open **Upload RTF**, select one `.rtf` file no larger than 2 MB, and convert.
The document may contain an explicit `Medline:` block or consist entirely of
one unambiguous, increasing numbered Ovid strategy. The byte-first parser
handles declared ANSI code pages, RTF Unicode controls, ignorable destinations,
and Word list labels before using the same parser and core as paste mode. An
example RTF template is downloadable from the tab; Title, End_date, PICO, and
other Evidentia metadata are not required.
...
`CITATION.cff` with approved person or organisation metadata. The public
repository URL is already recorded; no author identity is guessed here.

Software version: **0.1.0**. Conversion ruleset: **v20**. A suitable first
public tag after deployment acceptance is `v0.1.0`.
