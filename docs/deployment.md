...
Proposed public repository: `yuewu57/Evidentia-CSC`. The new UI and citation links
...
$archive = Join-Path (Split-Path (Get-Location) -Parent) "Evidentia_CSC_source_v1_YW_11092026.zip"
$dest = Join-Path (Split-Path (Get-Location) -Parent) "Evidentia-CSC"
...
Create an EMPTY GitHub repository named `Evidentia-CSC` under `yuewu57`, initially
...
git add .
git commit -m "Prepare Evidentia-CSC public research release"
if ($LASTEXITCODE -ne 0) { throw "git commit failed" }
git remote add origin https://github.com/yuewu57/Evidentia-CSC.git
...
| Setting | Value |
| --- | --- |
| Repository | yuewu57/Evidentia-CSC |
| Branch | main |
| Main file path | web/app.py |
| Advanced settings → Python version | 3.12 |
| App URL | Request evidentia-csc, subject to availability |
...
The proposed address `https://evidentia-csc.streamlit.app` is not reserved by
...
https://yuewu57.github.io/Evidentia-CSC/ . Verify the actual deployment URL.
